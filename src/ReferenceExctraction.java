...
public class ReferenceExctraction implements Serializable {
    ...
    public static void getIndividuals_sameAs_pair(String type) throws Exception {
        ...
        */
        ...
        result = readCache_sameAs_Individuals(cacheAddr);
        ...
        try{
            ...
            writeCache_sameAs_Individuals(result, cacheAddr);
        }
    ...
    private static void writeCache_sameAs_Individuals(ArrayList<ArrayList<String>> result, String cacheAddr) throws Exception {
        try {
            System.out.print("\n writing in sameAs individual cache...");
            ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(cacheAddr));
            oos.writeObject(result);
            oos.close();
        } catch(Exception e){
            System.out.print("\n cannot write in sameas individual cache/ may be path directory is not found...");
        }
    }
    private static ArrayList<ArrayList<String>> readCache_sameAs_Individuals(String cacheAddr) {

        ObjectInputStream ois = null;
        ArrayList<ArrayList<String>> sameAs_individualsCache = new ArrayList<>();

        try {
            System.out.print("\n reading from same as individual cache...");
            ois = new ObjectInputStream(new FileInputStream(cacheAddr));
            sameAs_individualsCache = (ArrayList<ArrayList<String>>) ois.readObject();
            ois.close();
        } catch (Exception e) {
            System.out.print("\n cannot read from sameAs individual cache...");
